pipeline {
  ...
  environment {
    ...
    GITHUB_REPO = "ergon-automation-labs/ergon-rpg"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/rpg_bot/bin/rpg_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyRpg.Release.create()'"
          $RELEASE_BIN eval 'BotArmyRpg.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyRpg.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyRpg.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_rpg/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_rpg/releases/start_erl.data)
        fi
